...
class PromocionController extends Controller
{
            ...
            if(isset($promos[0])){
                foreach ($promos as $promo){
                    ...
                    if(isset($promo->id)){
                        ...
                        if($promo->imagen_home){ @unlink(public_path().'/img/promociones/'.$promo->imagen_home); }
        ...
        try{
            if(isset($sugeridos[0])){
                foreach ($sugeridos as $sug){
                    ...
                    if(isset($promo->id)){
                        if($promo->imagen){ @unlink(public_path().'/img/promociones/'.$promo->imagen); }
                        if($promo->imagen_home){ @unlink(public_path().'/img/promociones/'.$promo->imagen_home); }
    ...
    public function promoinsta(Request $request)
    {
        $insta_id = $request->insta_id;
        $promo_id = $request->promo_id;
        $promoinsta = PromoInsta::where(['promo_id'=>$promo_id, 'insta_id' =>$insta_id])->first();
    ...
    public function open(Request $request)
    {
        $id    = $request->id;
        $promo = Promocion::find($id);
        $vista = view('dashboard.partials.promo', compact('promo'))->render();
